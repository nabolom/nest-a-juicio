...
required=(
  ...
  ".claude/commands/explicar-swarm.md"
...
grep -q 'resultados/reporte-swarm.md' scripts/correr-swarm.sh || { echo "Launcher Swarm no exige reporte final" >&2; exit 1; }
grep -q 'exactamente tres teammates' scripts/correr-swarm.sh || { echo "Launcher Swarm no está acotado para la demo" >&2; exit 1; }
...
grep -q '/decidir-arquitectura' README.md || { echo "README no explica la decisión guiada opcional" >&2; exit 1; }
grep -q '/explicar-swarm' README.md || { echo "README no explica el debrief automático" >&2; exit 1; }
grep -q 'resultados/reporte-swarm.md' README.md || { echo "README no explica el reporte Swarm" >&2; exit 1; }
...
echo "OK — ejercicio S5: Nest, Swarm breve con reporte y debrief automático ejecutables."
